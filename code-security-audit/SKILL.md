...
description: >
  漏洞分析任务路由器。用户提到安全审计、漏洞挖掘、风险分析、接口安全审查、入口/攻击面分析、sink 反查时触发。
  也适用于 "审一下这段代码"、"看看这个 handler"、"帮我挖漏洞"、"分析这个入口"、
  "这个 sink 点能不能利用"、"自顶向下"、"自底向上" 等场景。
  不适用于代码阅读理解、性能优化或功能开发。
  注意：本 skill 不自行分析代码，只负责解析任务并派发给专门的 agent。
---
